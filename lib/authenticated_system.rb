module AuthenticatedSystem
  ...
  def admin_required
    ...
    if user? and authorize?(session[:user]) and session[:user].has_role?("admin")
      return true
    end
  ...
  def moderator_required
    ...
    if user? and authorize?(session[:user]) and session[:user].has_role?("moderator", "admin")
      return true
    end
  ...
  def access_denied
    flash[:notice] = "Sign In or Register to Continue"
    redirect_to :controller => "/user", :action => "login", :to=>session['return-to']
  end
  ...
  def user?
    ...
    key = params[:key]
    if key
      session[:user] = User.authenticate_by_token(key)
      return true if not session[:user].nil?
    end
